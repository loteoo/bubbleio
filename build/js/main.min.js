  ...
  h('div', {class: "overlay"}, [
    ...
    h('form', {class: "bubbleForm", error: state.bubbleForm.error, onsubmit: ev => {
        ...
        h('fieldset', null, [
          ...
          h('input', {type: "text", name: "title", placeholder: "Bubble Name", required: true})
        ]),
        h('fieldset', null, [
          h('label', null, ["Unique name alias"]),
...
const BubbleItem = ({bubble, currentBubble}) =>
  h('li', {current: (bubble.name == currentBubble.name).toString(), userCount: bubble.userCount}, [
    Link({to: "/" + bubble.name}, [
      bubble.title,
      h('div', {class: "users"}, [
        h('div', {class: "icon"}),
  ...
  if (currentThread) {
    return (
      h('div', {class: "thread-view", _id: currentThread._id, bubble_id: currentThread.bubble_id, onupdate: (el, oldProps) => {
        ...
        h('div', {class: "frame", onscroll: ev => { if (isElementInViewport(ev.target.children[1].firstChild)) { console.log("Load more not working yet"); } }}, [

          Thread({currentThread: currentThread, currentBubble: currentBubble}),

          h('div', {class: "scroller"}, [
            h('div', {class: "conversation", messageCount: currentThread.messages.length, onupdate: (el, oldProps) => {
