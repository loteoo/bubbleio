...
const Keyboard = (state, actions) =>
  h('form', {class: "keyboard " + state.keyboardMode, onsubmit: ev => { actions.keyboardSubmit(ev); return false; }}, [
    h('div', {class: "expander " + state.keyboardStatus, onclick: ev => actions.expandKeyboard(state.keyboardStatus)}, [
...
const Thread$1 = (thread, index, currentBubble, actions) => {
  if (!thread.userCount) {
    thread.userCount = 0;
  }
  if (!thread.order) {
    ...
    return (
      h('div', {class: "thread-view", _id: currentThread._id, bubble_id: currentThread.bubble_id, messageCount: currentThread.messages.length, onupdate: (el, oldProps) => {
        ...
        h('div', {class: "frame", onscroll: (ev) => { if (isElementInViewport(ev.target.firstChild)) { console.log("Load more not working yet"); } }}, [
          ...
          Thread$1(currentThread, null, currentBubble, actions),
          h('ul', {class: "messages"}, [
