      ...
      h('ul', {class: "menu"}, [
        h('li', null, [
          ...
          h('ul', {class: "bubbles"}, [
            ...
            h('li', null, [h('span', null, ["Create bubble"])])
          ])
        ]),
        h('li', null, [
          h('span', null, ["Multibubbles"]),
          h('ul', null, [
            h('li', null, [h('span', null, ["Create Multibubble"])])
          ])
        ])
      ])
    ])
...
const BubbleItem = (bubble) =>
  h('li', null, [
    Link({to: "/" + bubble.name}, [bubble.title + " (" + bubble.userCount + ")"])
  ]);

const ThreadKeyboard = (state, actions) =>
  h('form', {class: "keyboard", 'data-mode': "default", onsubmit: ev => {
    ...
    h('div', {class: "expander", onclick: ev => ev.target.classList.contains("opened") ? ev.target.classList.remove("opened") : ev.target.classList.add("opened")}, [
      h('div', {class: "default", onclick: ev => { ev.target.parentElement.parentElement.dataset.mode = "default"; ev.target.parentElement.classList.remove("opened"); }}, ["x"]),
      h('div', {class: "text", onclick: ev => { ev.target.parentElement.parentElement.dataset.mode = "text"; ev.target.parentElement.classList.remove("opened"); }}, ["txt"]),
      h('div', {class: "link", onclick: ev => { ev.target.parentElement.parentElement.dataset.mode = "link"; ev.target.parentElement.classList.remove("opened"); }}, ["url"]),
      h('div', {class: "image", onclick: ev => { ev.target.parentElement.parentElement.dataset.mode = "image"; ev.target.parentElement.classList.remove("opened"); }}, ["pic"])
    ]),
...
const ThreadItem = (thread, index, currentBubble, actions) => {
  if (!thread.userCount) {
    thread.userCount = 0;
  }
  if (!thread.order) {
  ...
  if (window.innerWidth >= 768) { // If desktop
    return (
      h('li', {key: thread._id, class: "thread desktop", 'data-type': thread.type, 'data-upvoted': thread.upvoted, onclick: ev => {
    ...
    return (
      h('li', {key: thread._id, class: "thread", 'data-type': thread.type, 'data-upvoted': thread.upvoted, onclick: () => {
          actions.location.go("/" + currentBubble.name + "/" + thread._id);
        }, oncreate: el => {
          el.classList.add("slidein");
          setTimeout(() => {
...
const CurrentBubble = ({currentBubble, state, actions}) => {
  if (currentBubble) {
      ...
      h('div', {class: "bubble-view", name: currentBubble.name, onupdate: (el, oldProps) => {
        ...
        h('div', {class: "frame", onscroll: ev => { if (isElementInViewport(ev.target.lastChild)) {  console.log("Load more not working yet"); } }}, [
          h('div', {class: "bubble-header"}, [
            ...
            h('h2', null, [currentBubble.title + userCountTxt]),
            h('div', {class: "options"}, [
              h('button', {onclick: ev => {
              ...
              h('ul', null, [
                h('li', null, [h('span', null, ["Leave bubble"])])
              ])
            ])
          ]),
          h('ul', {class: "threads"}, [
...
const MessageKeyboard = (state, actions) =>
  h('form', {class: "keyboard", 'data-mode': "default", onsubmit: ev => {
      ev.preventDefault();
...
const Thread = (thread, index, currentBubble, actions) => {
  if (!thread.userCount) {
    thread.userCount = 0;
  }
  if (!thread.order) {
  ...
  return (
    h('li', {key: thread._id, class: "thread", 'data-type': thread.type, 'data-upvoted': thread.upvoted, oncreate: el => {
      el.classList.add("slidein");
      setTimeout(() => {
        el.classList.remove("slidein");
      }, index * 50);
...
const threadFooter$1 = (thread, actions) => (
  h("div", { class: "footer" }, [
    h("div", { class: "users", userCount: thread.userCount, onupdate: (el, oldProps) => {
      if (oldProps.userCount < thread.userCount) {
        el.classList.add("countUp");
