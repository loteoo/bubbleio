...
const Thread = (thread, index, currentBubble, actions, display = "summary") => {
  if (!thread.userCount) {
    thread.userCount = 0;
  }
  if (!thread.messages) {
  ...
  if (display == "summary") {
    return (
      h('li', {key: thread._id, class: "thread", 'data-type': thread.type, 'data-upvoted': thread.upvoted, 'data-display': display, onclick: () => {
    ...
    return (
      h('li', {key: thread._id, class: "thread", 'data-type': thread.type, 'data-upvoted': thread.upvoted, 'data-display': display, oncreate: el => {
        el.style.transitionDelay = index * 50 + "ms";
        el.classList.add("slidein");
        setTimeout(() => {
          el.classList.remove("slidein");
    ...
    return (
      h('li', {key: thread._id, class: "thread", 'data-type': thread.type, 'data-upvoted': thread.upvoted, 'data-display': display, onclick: ev => {
        actions.location.go("/" + currentBubble.name + "/" + thread._id);
      }, oncreate: el => {
        el.style.transitionDelay = index * 50 + "ms";
        el.classList.add("slidein");
