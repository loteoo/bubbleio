...
const ThreadItem = (thread, index, currentBubble, currentThread, actions) => {
  ...
  if (window.innerWidth >= 768) { // If desktop
    return (
      h('li', {key: thread._id, class: "thread desktop" + currentClass, index: index, 'data-type': thread.type, 'data-upvoted': thread.upvoted, onclick: ev => {
    ...
    return (
      h('li', {key: thread._id, class: "thread" + currentClass, index: index, 'data-type': thread.type, 'data-upvoted': thread.upvoted, onclick: () => {
          actions.location.go("/" + currentBubble.name + "/" + thread._id);
        }, oncreate: el => {
          el.classList.add("slidein");
          setTimeout(() => {
