...
const ThreadContent = ({thread, currentBubble}) => {
  if (thread.type == "default") {
    ...
    return h('div', {class: "text"}, [shortenText(thread.text, 250)]);
  } else if (thread.type == "link") {
    if (thread.url.match(/^(https?\:\/\/)?(www\.youtube\.com|youtu\.?be)\/.+$/)) { // If is youtube
      return (
        h('div', {class: "linkPreview youtubePreview"}, [
          h('img', {src: thread.thumbnail, alt: thread.title})
        ])
      )
    } else if (thread.url.match(/^(http\:\/\/|https\:\/\/)?(www\.)?(vimeo\.com\/)([0-9]+)$/)) {
      return (
        h('div', {class: "linkPreview vimeoPreview"}, [
          h('img', {src: thread.thumbnail, alt: thread.title})
        ])
      )
    }
    return h('a', {href: thread.url, target: "_blank", class: "link"}, [thread.url])
  } else if (thread.type == "image") {
    return h('div', {class: "img"}, [h('img', {src: thread.src, alt: thread.title})])
  }
};
...
const ThreadFullContent = ({thread, currentBubble}) => {
  if (thread.type == "default") {
    return
  } else if (thread.type == "text") {
    return h('div', {class: "text"}, [thread.text]);
  } else if (thread.type == "link") {
    if (thread.url.match(/^(https?\:\/\/)?(www\.youtube\.com|youtu\.?be)\/.+$/)) { // If is youtube
      return (
        h('a', {href: thread.url, target: "_blank", class: "linkPreview youtubePreview"}, [
          h('img', {src: thread.thumbnail, alt: thread.title})
        ])
      )
    } else if (thread.url.match(/^(http\:\/\/|https\:\/\/)?(www\.)?(vimeo\.com\/)([0-9]+)$/)) {
      return (
        h('a', {href: thread.url, target: "_blank", class: "linkPreview vimeoPreview"}, [
          h('img', {src: thread.thumbnail, alt: thread.title})
        ])
      )
    }
    ...
    return h('div', {class: "img"}, [h('a', {href: thread.src, target: "_blank"}, [h('img', {src: thread.src, alt: thread.title})])])
  }
};
